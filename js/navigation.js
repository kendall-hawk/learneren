// js/navigation.js - 增强版自定义导航系统 (含词频集成优化)
...
class Navigation {
    ...
    detectNodeType(item) {
        ...
        // 🎯 词频工具识别优先级最高
        if (this.isWordFrequencyTool(item)) {
            console.log('[Navigation] 🔤 检测到词频工具:', item.title || item.id);
    ...
    isWordFrequencyTool(item) {
        ...
        if (wordFreqIds.includes(item.id || item.seriesId)) {
            return true;
        }
        
        // 方法2：通过类型识别
        if (item.type === 'word-frequency' || item.type === 'word-freq') {
            return true;
        }
        
        // 方法3：通过标题识别
        const title = (item.title || item.series || '').toLowerCase();
        const titleKeywords = [
            '词频', 'word frequency', 'frequency analysis', 'vocab stats',
            'vocabulary analysis', '单词频率', 'word statistics', '词汇统计'
        ];
        ...
        // 方法4：通过URL识别
        if (item.url && (item.url.includes('word-frequency') || item.url.includes('word-freq'))) {
            return true;
        }
        
        // 方法5：通过自定义属性识别
        if (item.customProps?.isWordFrequency || item.wordFrequency || item.isWordFreq) {
            return true;
        }
    ...
    createNavigationItem(node) {
        ...
        // 🎯 词频工具特殊图标
        const wordFreqIcon = node.type === 'word-frequency' ? '🔤' : '';
        const finalIcon = wordFreqIcon || node.icon || '';
        const finalIconHtml = finalIcon ? `<span class="nav-icon">${finalIcon}</span>` : iconHtml;
    ...
    getItemClasses(node, isExpandable) {
        ...
        // 🎯 词频工具样式
        if (node.type === 'word-frequency') {
            classes.push('word-frequency-item');
        }
    ...
    handleNavItemClick(itemId, clickedElement = null) {
        ...
        // 🎯 词频工具优先处理
        if (node.type === 'word-frequency') {
            console.log('[Navigation] 🔤 处理词频工具点击');
            this.handleWordFrequencyNavigation(node);
            return;
        }
    ...
    // 🎯 新增：专用词频工具处理
    handleWordFrequencyNavigation(node) {
        try {
            console.log('[Navigation] 🚀 开始处理词频工具导航');
            
            // 关闭侧边栏
            this.close();
            
            // 更新页面标题
            this.updateTitle('词频分析工具');
            ...
            console.log('[Navigation] ✅ 词频工具导航处理完成');
            
        } catch (error) {
            console.error('[Navigation] ❌ 词频工具导航失败:', error);
            
            // 降级处理：显示错误信息
            this.displayWordFrequencyError(error, node);
        }
    }

    // 🎯 新增：词频工具错误处理
    displayWordFrequencyError(error, node) {
        const errorMessage = `
            <div class="error-message" style="text-align: center; padding: 40px; color: #dc3545; background: #fff5f5; border-radius: 8px; margin: 20px;">
                <div style="font-size: 3rem; margin-bottom: 20px;">🔤</div>
                <h3>词频分析工具暂不可用</h3>
                <p>工具启动时出现错误：</p>
                <p style="font-style: italic; color: #6c757d; margin: 16px 0;">${error.message}</p>
                <div style="margin-top: 24px;">
                    ...
                    <button onclick="window.app?.navigation?.handleAllArticlesNavigation?.()" 
        ...
        if (this.contentArea) {
            this.contentArea.innerHTML = errorMessage;
        }
        ...
        this.dispatchEvent('navigationError', {
            ...
            nodeId: node.id,
            nodeType: node.type,
            source: 'word-frequency-navigation'
        });
    }
    ...
    addEnhancedStyles(container) {
        ...
        style.textContent = `
            ...
            /* 🎯 词频工具导航项（简化，样式由工具本身处理） */
            .nav-item.word-frequency-item {
                /* 只保留基础识别类名，具体样式由词频工具UI处理 */
            }
    ...
    getEnhancedNavigationStats() {
        return {
            ...
            enhancedFeatures: {
                ...
                wordFrequencyIntegration: true // 🎯 新增
            }
        };
    }
    ...
    getNavigationStats() {
        return {
            ...
            wordFrequencyToolsCount: this.getWordFrequencyToolsCount() // 🎯 新增
        };
    }

    // 🎯 新增：统计词频工具数量
    getWordFrequencyToolsCount() {
        let count = 0;
        this.walkNavigationTree(this.state.navigationTree, (node) => {
            if (node.type === 'word-frequency') {
                count++;
...
window.navigateToWordFrequency = function() {
    if (window.app && window.app.navigation) {
        // 🎯 优先查找词频工具节点
        const wordFreqNode = window.app.navigation.findWordFrequencyNode();
        if (wordFreqNode) {
            window.app.navigation.handleWordFrequencyNavigation(wordFreqNode);
            return true;
        } else {
            console.warn('[Navigation] 未找到词频工具节点');
            return false;
        }
    }
    return false;
...
// 🎯 新增：查找词频工具节点的便捷方法
Navigation.prototype.findWordFrequencyNode = function() {
    let wordFreqNode = null;
    this.walkNavigationTree(this.state.navigationTree, (node) => {
        if (node.type === 'word-frequency' && !wordFreqNode) {
            wordFreqNode = node;
        }
    });
    return wordFreqNode;
...
window.debugEnhancedNavigation = function() {
    if (window.app && window.app.navigation) {
        ...
        console.log('🔤 词频工具数量:', nav.getWordFrequencyToolsCount());
...
window.addEnhancedNavigationListener = function(eventType, callback) {
    const supportedEvents = [
        ...
        'wordFrequencyRequested' // 🎯 词频工具请求时
    ];
...
console.log('[Navigation] ✅ 增强版自定义导航系统加载完成 (含词频集成)');
console.log('[Navigation] 🚀 新功能: 位置对齐子菜单、词频工具集成、增强动画、响应式设计');
